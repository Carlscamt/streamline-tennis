...
import numpy as np
import joblib
from feature_engineering_fixed_no_leakage import EloRating, Glicko2Rating
...
def load_model_and_data():
    try:
        model = joblib.load('tennis_model_no_leakage.joblib')  # Using corrected leak-free model
        historical_data = pd.read_csv('tennis_data_features_no_leakage.csv')  # Using leak-free features
        historical_data['Date'] = pd.to_datetime(historical_data['Date'])
        return model, historical_data
    except FileNotFoundError:
        st.error("Model or data files not found. Running model training first...")
        import train_model_no_leakage
        train_model_no_leakage.train_model_no_leakage()
        ...
        if submit:
            if player1 and player2 and surface:
                if player1 == player2:
                    ...
                    feature_cols = ['career_win_pct_diff', 'surface_win_pct_diff', 'recent_form_diff', 'h2h_win_pct_diff', 'ranking_diff', 'elo_rating_diff', 'glicko2_rating_diff', 'glicko2_rd_diff', 'glicko2_volatility_diff']
